...
import {
  db_getStoreReservations,
  db_getStoreData,
  db_confirmReservation,
  db_reserveReservationSlot
} from '../utils/db_ops'
...
exports.next = function (req: Request, res: Response) {
  ...
  db_getStoreData(DB, store_id, function(result: any) {
    if (result == 0) {
      ...
      db_getStoreReservations(DB, store_id, date_today, function(
        result_2: any
      ) {
        let reservations = result_2
        ...
        if (reservation_time != 0) {
          db_reserveReservationSlot(
            ...
            function(result: any) {
              if (result.length != 0) {
                res.end(
                  JSON.stringify({ success: true, reservationDetails: result })
                )
              } else {
                res.end(
                  JSON.stringify({
